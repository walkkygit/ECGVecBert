...
Usage:
    python vqvae_bert_finetuning_build_sentences_launcher.py --all
...
CNN_EMBED_TYPE = None     # 'vqvae_encoder'
...
def launch(in_channels: int, use_frac: float, cnn_embed_type: str):
    sm_session, ROLE_ARN = get_session_and_role()
    hp = {"in_channels": in_channels,
          "batch_size": 32,
          "use_frac": use_frac,
        }
...
if __name__ == "__main__":
    ...
    if args.all:
        for frac in USE_FRAC:
            launch(in_channels, frac, cnn_embed_type)
    else:
        use_frac = args.use_frac if args.use_frac is not None else USE_FRAC[-1]
        launch(in_channels, use_frac, cnn_embed_type)
