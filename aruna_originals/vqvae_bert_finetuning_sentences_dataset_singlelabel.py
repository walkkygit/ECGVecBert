...
from xresnet1d101_features_ecg_waveforms import ECGSegmentEmbedder
...
model_prefix = "aruna-files/vqvae_final_12lead_singlelabel/vqvae"
...
def read_ecg_signals_from_s3(dataset_name: str) -> dict:
    ...
    dict with:
        ...
        labels     : (N,) array
        ids        : (N,) array
        strat_fold : (N,) int array — PTB-XL only, else absent
    """
    ...
    with s3_fs.open(s3_path, "rb") as f:
        ...
        if dataset_name == "ptbxl":     # id used should be record id, not patient id as saved
...
def read_ecg_data(dataset_name, use_frac, train_frac=0.7, val_frac=0.1, test_frac=0.2, min_class_count=15, seed=42) -> Splits:
    ...
    if dataset_name == "ptbxl":
        # Use official split 70/10/20
        strat_fold = store["strat_fold"]
        train_idx = all_idx[strat_fold <= 7]
        val_idx   = all_idx[strat_fold == 8]
        test_idx  = all_idx[np.isin(strat_fold, [9, 10])]
    else:
        # Fixing the seed produces the same split each time
        # Stratified split by label: each record id has exactly one
        # label, train_test_split keeps label
        # proportions consistent across train/val/test.
        df = pd.DataFrame({'id': ids, 'label': labels})
        id_labels = df.drop_duplicates(subset='id').set_index('id')['label']
    ...
    def _select(idx):
        ...
        y = labels[idx]
        id_sel = ids[idx]           # one record per patient, except for ptbxl 
        return X, y, id_sel
...
def assign_finetuning_codebook_indices(
    ...
    in_channels: int,
    batch_size: int = 256,
    embedders: dict[str, ECGSegmentEmbedder] | None = None,
    ...
    embedders : optional {segment: ECGSegmentEmbedder}
    ...
    for split_name, (ids, X, y) in splits.items():
        ...
        results: dict[str, dict] = {
            seg: {
                "beat_idxs": [], "ecg_idxs": [], "token_indices": [], "labels": [],
                "embeddings": [] if embedders is not None else None,
            }
            for seg in SEGMENTS
        }
        ...
        def _flush(seg: str) -> None:
            ...
            if embedders is not None:
                result["embeddings"].append(_embed_batch(embedders[seg], waveforms))
...
def build_finetuning_beat_sentences(
    ...
    dict with one row per sentence:
        ...
        xresnet_embeddings : (N, MAX_SENTENCE_LEN, D) float16  — only when present
    """
    ...
    if has_embeddings:
        result["xresnet_embeddings"] = np.stack(xresnet_list)
    return result
...
def _build_save_kwargs(sentences: dict) -> dict:
    ...
    if "xresnet_embeddings" in sentences:
        kwargs["xresnet_embeddings"] = sentences["xresnet_embeddings"]
    return kwargs
...
def save_finetuning_sentences(
    ...
    row_keys  = {"sentence_tokens", "labels", "ecg_idxs", "num_beats",
                 "beat_idx_start", "beat_idx_end", "xresnet_embeddings"}
...
def get_or_build_finetuning_sentences(
    ...
    #xresnet_model_path = "./models/fastai_xresnet1d101.pth"
    xresnet_model_path = f"s3://{bucket_out}/{model_prefix}/models/fastai_xresnet1d101.pth"
    ...
    models:   dict[str, torch.nn.Module]      = {}
    embedders: dict[str, ECGSegmentEmbedder]  = {}
    for seg in SEGMENTS:
        signal_len = SIGNAL_LENS[seg]
        embedders[seg] = ECGSegmentEmbedder(
            signal_len=signal_len,
            in_channels=in_channels,
            model_path=xresnet_model_path,
            device=str(device),
        )
    ...
    seg_split_results, codebook_sizes = assign_finetuning_codebook_indices(
        ...
        batch_size=batch_size, embedders=embedders,
    )
...
class FinetuningBeatSentenceDataset(Dataset):
    ...
    num_beats           : () int64
    labels              : () int64                   — ECG-level class label
    ecg_idxs            : () str                   - patient record id
    xresnet_embeddings  : (MAX_SENTENCE_LEN, D) float32  — only when present in the file
    """
    ...
    def __init__(self, sentences: dict) -> None:
        ...
        self._labels  = sentences["labels"]
        self._ecg_idxs = sentences["ecg_idxs"].astype(np.str_)
        self._xresnet = sentences.get("xresnet_embeddings")
        self.vocab_size    = int(sentences["vocab_size"])
        self.vocab_offsets = sentences["vocab_offsets"]
        self.xresnet_embedding_dim = (
            int(self._xresnet.shape[-1]) if self._xresnet is not None else 0
        )
        log.info(
            f"FinetuningBeatSentenceDataset: {len(self._tokens):,} sentences"
            + (f", xresnet_dim={self.xresnet_embedding_dim}" if self._xresnet is not None else "")
        )
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self._xresnet is not None:
            item["xresnet_embeddings"] = torch.tensor(
                self._xresnet[idx].astype(np.float32), dtype=torch.float32
            )
        return item
...
def main(in_channels: int, use_frac: float, batch_size: int) -> None:
    """
    Generate fine-tuning sentences for all 3 datasets and pull one batch from
    ...
    for dataset_name in DATASETS:
        ...
        for split_name in ("train", "val", "test"):
            ...
            log.info(f"    label          : {int(batch['labels'][0])}")
            if "xresnet_embeddings" in batch:
                emb = batch["xresnet_embeddings"][0].float()
                log.info(
                    f"    xresnet_emb    : shape={tuple(emb.shape)}"
...
def _load_single_shard(
    ...
    with s3_fs.open(s3_path, "rb") as f:
        ...
        if "xresnet_embeddings" in data:
            result["xresnet_embeddings"] = data["xresnet_embeddings"]
    return result
...
if __name__ == "__main__":
    ...
    main(args.in_channels, args.use_frac, args.batch_size)