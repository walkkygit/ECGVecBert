...
Phase 3 (--folds yes): everything under split_3_folds/ instead; --run_tag may list several tags (comma-separated),
e.g. the 3 fold models -> per-model metrics + one probability-averaged ensemble in results/<sub>/ensemble__<tags>/.
"""
...
def launch(run_tag: str, seeds: str, split: str, confirm: str, force: str, instance_type: str, folds: str = "no"):
    boto_session = boto3.Session()
    sm_session = sagemaker.Session(boto_session=boto_session)
    role = sagemaker.get_execution_role()
    hp = {"run_tag": run_tag, "seeds": seeds, "split": split, "i_confirm_final_7k": confirm, "force": force, "folds": folds}
    logs_prefix = S3_OUTPUT_PREFIX_FOLDS if folds == "yes" else S3_OUTPUT_PREFIX
    short = re.sub(r"[^a-zA-Z0-9-]", "-", f"eval-{'7k' if split == '7k' else 'int'}-{'f-' if folds == 'yes' else ''}{run_tag}")[:38].strip("-")
    estimator = PyTorch(
        ...
        base_job_name=short,
        output_path=f"s3://{BUCKET_OUT}/{logs_prefix}/",
        code_location=f"s3://{BUCKET_OUT}/{logs_prefix}/code",   # source tarballs here, not at the bucket root
        hyperparameters=hp,
        volume_size=50,
        max_run=3 * 3600,
    )
...
if __name__ == "__main__":
    ...
    p.add_argument("--folds", type=str, default="no", choices=["no", "yes"], help="'yes' = phase 3 (split_3_folds)")
    a = p.parse_args()
    if a.split == "7k" and a.i_confirm_final_7k != "yes":
        p.error("--split 7k is the one locked final evaluation; add --i_confirm_final_7k yes")
    launch(a.run_tag, a.seeds, a.split, a.i_confirm_final_7k, a.force, a.instance_type, folds=a.folds)
